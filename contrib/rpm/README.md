...
The RPM spec file provided here is for Ruxcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://ruxcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Ruxcoin git repository](https://github.com/ruxcoin/ruxcoin) but are not part of
...
Currently only `Source10` is used, for the example `ruxcoin.conf` file.
...
in the ruxcoin git repository. At present only an SVG version of the Ruxcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the ruxcoin
...
needed when 0.12.1 is released, a proper fix is already in the Ruxcoin git
master branch.
...
idea to build Ruxcoin Core against a static version of that library compiled
...
The `ruxcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `ruxcoin-server` package. At this point, most RPM based
...
The `ruxcoin-server` package should have SELinux support. How to properly do
...
Ruxcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/ruxcoin/). He has been
packaging Ruxcoin for Fedora at least since 2012.
...
4. I renamed the `ruxcoin` package that contains the Qt GUI to `ruxcoin-core` as
that appears to be how the general population refers to it, in contrast to
`ruxcoin-xt` or `ruxcoin-classic`. I wanted to make sure the general population

...
###   This script attempts to download the signature file SHA256SUMS.asc from ruxcoin.org
...
WORKINGDIR="/tmp/ruxcoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://ruxcoin.org/bin/"
VERSIONPREFIX="ruxcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'ruxcoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Ruxcoin Core binary release signing key installed?"
   fi
